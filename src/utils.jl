function sample_collocation_points(domain::T, num_points::Tuple{Int, Int}, sampler::SamplingAlgorithm) where {T}
    d = length(domain)
    ni, nb = num_points
    ...
    for j=1:nbcs
        ...
        lb_local = lb[mask]; ub_local = ub[mask]
        push!(boundary_points, QuasiMonteCarlo.sample(nb_local, lb_local, ub_local, sampler))
    end
